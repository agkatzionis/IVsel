...
beta.r <- 0.5
gamma.r <- 0.4
delta.r <- 0   ## Direct effect replaced by Y-R confounding.
lamda.y <- 0.5   ## U-Y effect (semi-arbitrary - R2 = 0.20).
lamda.r <- 0.5   ## U-R effect (McFadden's R2 = 0.04).
...
colnames(diagnostics) <- c("Strength", "Strength | X", "Strength | X, Y", "Prop Selected", "MD X", "OR X", 
                           "MD Y", "OR Y", "Wmin | X", "Wmax | X", "Wmin | X, Z", "Wmax | X, Z", "R2 (Y ~ U)",
                           "McFadden Z", "McFadden X", "McFadden Y", "McFadden XY", "McFadden U")
...
for (I in 1:iter) {
  ...
  Z <- rnorm(n, 0, 1)
  U <- rnorm(n, 0, 1)
  Y <- alpha.y + X * beta.y + U * lamda.y + rnorm(n, 0, 1)
  
  ## Simulate the selection coefficient.
  R.probs <- expit(alpha.r + beta.r * X + gamma.r * Z + delta.r * Y + lamda.r * U)
  ...
  diagnostics[I, 13] <- summary(lm(Y ~ U))$r.sq
  ...
  diagnostics[I, 18] <- 1 - glm(R ~ U, family = binomial)$deviance / glm(R ~ 1, family = binomial)$deviance
