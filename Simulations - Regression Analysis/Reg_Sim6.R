...
alpha.y <- 1
beta.y <- 0.1
alpha.r <- - 0.25 * exp(1)   ## So that 50% are selected.
beta.r <- 0.5
gamma.r <- 0.4
delta.r <- 0.25   ## Gives an OR of 2.2 (Y>e vs Y<e).
...
for (I in 1:iter) {
  ...
  oracle.est <- summary(glm(Y ~ X, family = "poisson"))
  
  ## Inverse probability weighting.
  ipw.est1 <- ipw.poisson(X, Y, R)
  ipw.est2 <- ipw.poisson(X, Y, R, Z)
  ipw.est3 <- ipw.poisson(X, Y, R, Z, drop.x = TRUE)
  ...
  ## We keep the implementation of Heckman models but will probably not include
  ## them in the paper since the "selection" function cannot model these properly.
  
  ## Partial likelihood MLE.
  partial.opt1 <- optim(par = rep(0, 3), partial.poisson.lik1, method = "BFGS", control = list(fnscale = -1), X = X, R = R, Z = Z)
  partial.opt2 <- optim(par = rep(0, 4), partial.poisson.lik2, method = "BFGS", control = list(fnscale = -1), X = X, R = R, Z = Z, Y = Y, alpha.hat = partial.opt1$par, hessian = TRUE)
  partial.sd <- sqrt(diag(solve(- partial.opt2$hessian))[1:2])
  
  ## Full likelihood MLE.
  full.opt <- optim(par = rep(0, 7), full.poisson.lik, method = "BFGS", control = list(fnscale = -1), X = X, R = R, Z = Z, Y = Y, hessian = TRUE)
  ...
  diagnostics[I, 7] <- mean(R[Y > exp(1)]) - mean(R[Y < exp(1)])
  diagnostics[I, 8] <- (mean(R[Y > exp(1)]) * (1 - mean(R[Y < exp(1)]))) / (mean(R[Y < exp(1)]) * (1 - mean(R[Y > exp(1)])))
