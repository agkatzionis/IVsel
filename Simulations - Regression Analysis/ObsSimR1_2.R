...
##########   OBSERVATIONAL SIMULATION R1-2   ##########

## Apply the IVsel methods in observational analyses
## with a varying strength of the Z-Y association.
...
filename <- paste("ObsSimR1_2_", range, "_results.RData", sep = "")

## Set up the range of Z-Y values.
gamma.y.values <- c(0.1, 0.2, 0.3, 0.5, 1)
## or tune these to represent proportions of variation in X explained by Z.
...
gamma.r <- 0.4
delta.r <- 0.5
gamma.y <- gamma.y.values[range]   ## Z-Y effect varied in this simulation.
...
colnames(diagnostics) <- c("Strength", "Strength | X", "Strength | X, Y", "Prop Selected", "MD X", "OR X", 
                           "MD Y", "OR Y", "Wmin | X", "Wmax | X", "Wmin | X, Z", "Wmax | X, Z", "R2 (X ~ Z)",
...
for (I in 1:iter) {
  ...
  X <- rnorm(n, 0, 1)
  Z <- rnorm(n, 0, 1)
  Y <- alpha.y + X * beta.y + Z * gamma.y + rnorm(n, 0, 1)
  ...
  diagnostics[I, 13] <- summary(lm(Y ~ Z))$r.sq
