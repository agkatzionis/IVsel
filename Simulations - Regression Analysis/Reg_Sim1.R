...
alpha.y <- 1   ## Arbitrary.
beta.y <- 0.1   ## Calibrated for power comparison.
alpha.r <- -0.5   ## So that 50% are selected.
beta.r <- 0.5   ## Semi-arbitrary (reasonable X-R association, McFadden's R2 = 0.04).
gamma.r <- 0.4   ## McFadden's pseudo-R2 = 0.02.
delta.r <- 0.5   ## Semi-arbitrary (reasonable Y-R association, McFadden's R2 = 0.04).
...
for (I in 1:iter) {
  ...
  if (I %% 100 == 0) save(alpha.y, beta.y, alpha.r, beta.r, gamma.r, delta.r, n, 
                          oracle, naive, partial, full, ipw1, ipw2, ipw3, 
                          heckman1, heckman2, heckman3, heckman4, diagnostics, 
                          file = filename)
