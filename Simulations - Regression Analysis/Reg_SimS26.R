...
alpha.y <- 1
beta.y <- 0.1
alpha.r <- -0.615  ## So that 50% are selected.
beta.r <- 0.5
gamma.r <- 0.23  ## McFadden's pseudo-R2 = 0.002.
delta.r <- 0.5
...
colnames(diagnostics) <- c("Strength", "Strength | X", "Strength | X, Y", "Prop Selected", "MD X", "OR X", 
                           "MD Y", "OR Y", "Wmin | X", "Wmax | X", "Wmin | X, Z", "Wmax | X, Z", 
...
for (I in 1:iter) {
  ...
  diagnostics[I, 13] <- 1 - glm(R ~ Z, family = binomial)$deviance / glm(R ~ 1, family = binomial)$deviance
  diagnostics[I, 14] <- 1 - glm(R ~ X, family = binomial)$deviance / glm(R ~ 1, family = binomial)$deviance
  diagnostics[I, 15] <- 1 - glm(R ~ Y, family = binomial)$deviance / glm(R ~ 1, family = binomial)$deviance
  diagnostics[I, 16] <- 1 - glm(R ~ X + Y, family = binomial)$deviance / glm(R ~ 1, family = binomial)$deviance
