...
alpha.y <- 1
beta.y <- 0.1
alpha.r <- - expit(1)   ## So that 50% are selected.
beta.r <- 0.5
gamma.r <- 0.4
delta.r <- 1   ## Gives an OR of 1.7 (Y=1 vs Y=0).

## Store results here.
oracle <- matrix(0, iter, 4); colnames(oracle) <- c("Intercept", "se.int", "Beta", "se.beta")
naive <- oracle; ipw1 <- oracle; ipw2 <- oracle; full <- oracle
...
colnames(diagnostics) <- c("Strength", "Strength | X", "Strength | X, Y", "Prop Selected", "MD X", "OR X",
                           "MD Y", "OR Y", "Wmin | X", "Wmax | X", "Wmin | X, Z", "Wmax | X, Z", "Y Prop", 
...
for (I in 1:iter) {
  ...
  X <- rnorm(n, 0, 1)
  Z <- rnorm(n, 0, 1)
  Y.probs <- expit(alpha.y + X * beta.y)
  ...
  oracle.est <- summary(glm(Y ~ X, family = "binomial"))
  
  ## Inverse probability weighting.
  ipw.est1 <- ipw.logistic(X, Y, R)
  ipw.est2 <- ipw.logistic(X, Y, R, Z)
  ipw.est3 <- ipw.logistic(X, Y, R, Z, drop.x = TRUE)
  
  ## Heckman's method.
  Yobs <- Y;  Yobs[R == 0] <- NA; Yobs <- as.logical(Yobs)
  heck1 <- summary(selection(R ~ Z, Yobs ~ X, method = "2step"))
  heck2 <- try(summary(selection(R ~ Z, Yobs ~ X, method = "ml")), silent = TRUE)
  heck3 <- summary(selection(R ~ X + Z, Yobs ~ X, method = "2step"))
  heck4 <- try(summary(selection(R ~ X + Z, Yobs ~ X, method = "ml")), silent = TRUE)
  ## Only the "ml" fits work with binary outcomes.
  
  ## There is no partial fit for the logistic case.
  
  ## Full likelihood MLE.
  full.opt <- optim(par = rep(0, 7), full.logit.lik, method = "BFGS", control = list(fnscale = -1), X = X, R = R, Z = Z, Y = Y, hessian = TRUE)
  ...
  if (class(heck2) != "try-error") heckman2[I, ] <- c(heck2$estimate[3, 1:2], heck2$estimate[4, 1:2])
  heckman3[I, ] <- c(heck3$estimate[4, 1:2], heck3$estimate[5, 1:2])
  if (class(heck4) != "try-error") heckman4[I, ] <- c(heck4$estimate[4, 1:2], heck4$estimate[5, 1:2])
  ...
  diagnostics[I, 7] <- mean(R[Y == 1]) - mean(R[Y == 0])
  diagnostics[I, 8] <- (mean(R[Y == 1]) * (1 - mean(R[Y == 0]))) / (mean(R[Y == 0]) * (1 - mean(R[Y == 1])))
  ...
  diagnostics[I, 13] <- mean(Y)
  diagnostics[I, 14] <- 1 - glm(R ~ Z, family = binomial)$deviance / glm(R ~ 1, family = binomial)$deviance
  diagnostics[I, 15] <- 1 - glm(R ~ X, family = binomial)$deviance / glm(R ~ 1, family = binomial)$deviance
  diagnostics[I, 16] <- 1 - glm(R ~ Y, family = binomial)$deviance / glm(R ~ 1, family = binomial)$deviance
  diagnostics[I, 17] <- 1 - glm(R ~ X + Y, family = binomial)$deviance / glm(R ~ 1, family = binomial)$deviance
  ...
  if (I %% 100 == 0) save(alpha.y, beta.y, alpha.r, beta.r, gamma.r, delta.r, n, oracle, naive,
                          full, diagnostics, ipw1, ipw2, ipw3, 
