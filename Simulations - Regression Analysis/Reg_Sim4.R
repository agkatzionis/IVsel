...
alpha.y <- 1
beta.y <- 0.1
alpha.r <- -1.025   ## So that 50% are selected.
beta.r <- 1
gamma.r <- 0.4
delta.r <- 0.5
...
for (I in 1:iter) {
  ...
  diagnostics[I, 5] <- mean(R[X == 1]) - mean(R[X == 0])
  diagnostics[I, 6] <- (mean(R[X == 1]) * (1 - mean(R[X == 0]))) / (mean(R[X == 0]) * (1 - mean(R[X == 1])))
