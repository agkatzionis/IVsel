...
##########   OBSERVATIONAL SIMULATION R1-1   ##########

## Apply the IVsel methods in observational analyses
## with a varying strength of the Z-X association.
...
filename <- paste("ObsSimR1_1_", range, "_results.RData", sep = "")

## Set up the range of Z-X values.
beta.x.values <- c(0.1, 0.2, 0.3, 0.5, 1)
...
gamma.r <- 0.4
delta.r <- 0.5
beta.x <- beta.x.values[range]   ## Z-X effect varied in this simulation.
...
for (I in 1:iter) {
  ...
  diagnostics[I, 13] <- summary(lm(X ~ Z))$r.sq
