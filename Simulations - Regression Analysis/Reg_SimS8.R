...
gamma.r <- 0.4
delta.r <- 0.5
gamma.y <- 1/3   ## Changes with beta.y to maintain 10% of variation in Y explained by Z.
...
colnames(diagnostics) <- c("Strength", "Strength | X", "Strength | X, Y", "Prop Selected", "MD X", "OR X", 
                           "MD Y", "OR Y", "Wmin | X", "Wmax | X", "Wmin | X, Z", "Wmax | X, Z", "R2 (Y ~ Z)",
