...
gy <- matrix(0, iter, 4); colnames(gy) <- c("Intercept", "se.int", "Beta", "se.beta")
...
for (I in 1:iter) {
  ...
  Y1 <- beta.y * X1 + gamma.y * U1 + rnorm(n1, 0, 1)   ## Won't be used anywhere.
  ...
  gy.est <- summary(lm(Y2 ~ G2))
  
  ## Naive and oracle estimates.
  naive.est <- summary(lm(X1[R1 == 1] ~ G1[R1 == 1]))
  oracle.est <- summary(lm(X1 ~ G1))
  
  ## Inverse probability weighting.
  ipw.est1 <- ipw.linear(X = G1, Y = X1, R = R1)
  ipw.est2 <- ipw.linear(X = G1, Y = X1, R = R1, Z = Z1)
  ipw.est3 <- ipw.linear(X = G1, Y = X1, R = R1, Z = Z1, drop.x = TRUE)
  
  ## Heckman's method.
  X1obs <- X1;  X1obs[R1 == 0] <- NA
  heck1 <- summary(selection(R1 ~ Z1, X1obs ~ G1, method = "2step"))
  heck2 <- summary(selection(R1 ~ Z1, X1obs ~ G1, method = "ml"))
  heck3 <- summary(selection(R1 ~ G1 + Z1, X1obs ~ G1, method = "2step"))
  heck4 <- summary(selection(R1 ~ G1 + Z1, X1obs ~ G1, method = "ml"))
  
  ## Partial likelihood MLE.
  partial.opt1 <- optim(par = rep(0, 3), partial.lik1, method = "BFGS", control = list(fnscale = -1), X = G1, R = R1, Z = Z1)
  partial.opt2 <- optim(par = rep(0, 5), partial.lik2, method = "BFGS", control = list(fnscale = -1), X = G1, R = R1, Z = Z1, Y = X1, alpha.hat = partial.opt1$par, hessian = TRUE)
  partial.sd <- sqrt(diag(solve(- partial.opt2$hessian))[1:2])
  
  ## Full likelihood MLE.
  full.opt <- optim(par = rep(0, 8), full.lik, method = "BFGS", control = list(fnscale = -1), X = G1, R = R1, Z = Z1, Y = X1, hessian = TRUE)
  full.sd <- sqrt(diag(solve(- full.opt$hessian))[1:2])
  
  ## Store G-Y results.
  gy[I, ] <- c(gy.est$coef[1, 1:2], gy.est$coef[2, 1:2])
  ...
  diagnostics[I, 13] <- mean(R1[Z1 > 0]) - mean(R1[Z1 < 0])
  diagnostics[I, 14] <- (mean(R1[Z1 > 0]) * (1 - mean(R1[Z1 < 0]))) / (mean(R1[Z1 < 0]) * (1 - mean(R1[Z1 > 0])))
  ...
  diagnostics[I, 21] <- 1 - glm(R1 ~ X1, family = binomial)$deviance / glm(R1 ~ 1, family = binomial)$deviance
  diagnostics[I, 22] <- 1 - glm(R1 ~ Y1, family = binomial)$deviance / glm(R1 ~ 1, family = binomial)$deviance
  diagnostics[I, 23] <- 1 - glm(R1 ~ X1 + Y1, family = binomial)$deviance / glm(R1 ~ 1, family = binomial)$deviance
  
  ## Print progress status.
  if (I %% 20 == 0) print(paste(I, " done."))
  
  ## Save results.
  if (I %% 100 == 0) save(beta.x, gamma.x, beta.y, gamma.y, alpha.r, beta.r, gamma.r, delta.r, n1, n2, gy, oracle, naive,
