...
## Non-linear first-stage model, population differences.
...
for (I in 1:iter) {
  ...
  Z1 <- rnorm(n1, 0, 1)
  G1 <- rnorm(n1, 0, 1)
  X1 <- beta.x * G1 + beta.x2 * G1^2 + gamma.x * U1 + rnorm(n1, 0, 1)
  
  ## Simulate the second sample.
  U2 <- rnorm(n2, 0, 1)
  Z2 <- rnorm(n2, 0, 1)
  G2 <- rt(n2, df = 4)
  X2 <- beta.x * G2 + beta.x2 * G2^2 + gamma.x * U2 + rnorm(n2, 0, 1)
  Y2 <- beta.y * X2 + gamma.y * U2 + rnorm(n2, 0, 1)
  
  ## Simulate the selection coefficient.
  R2.probs <- expit2(alpha.r + beta.r * X2 + gamma.r * Z2 + delta.r * Y2)   ## Using expit2 instead of expit for numerical stability.
  ...
  heck1 <- try(summary(selection(R2 ~ Z2, Y2obs ~ G2, method = "2step")), silent = TRUE)
  heck2 <- try(summary(selection(R2 ~ Z2, Y2obs ~ G2, method = "ml")), silent = TRUE)
  heck3 <- try(summary(selection(R2 ~ G2 + Z2, Y2obs ~ G2, method = "2step")), silent = TRUE)
  heck4 <- try(summary(selection(R2 ~ G2 + Z2, Y2obs ~ G2, method = "ml")), silent = TRUE)
  ...
  if (class(heck1)[1] != "try-error") heckman1[I, ] <- c(heck1$estimate[3, 1:2], heck1$estimate[4, 1:2])
  if (class(heck2)[1] != "try-error") heckman2[I, ] <- c(heck2$estimate[3, 1:2], heck2$estimate[4, 1:2])
  if (class(heck3)[1] != "try-error") heckman3[I, ] <- c(heck3$estimate[4, 1:2], heck3$estimate[5, 1:2])
  if (class(heck4)[1] != "try-error") heckman4[I, ] <- c(heck4$estimate[4, 1:2], heck4$estimate[5, 1:2])
  ...
  ## Save results.
  if (I %% 100 == 0) save(beta.x, beta.x2, gamma.x, beta.y, gamma.y, alpha.r, beta.r, gamma.r, delta.r, n1, n2, gx, oracle, naive,
