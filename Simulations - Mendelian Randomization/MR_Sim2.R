...
gamma.y <- 1
alpha.r <- 0
beta.r <- 1   ## Semi-arbitrary (McFadden's R2 = 0.20).
gamma.r <- 0.5
...
for (I in 1:iter) {
  ...
  gy.est <- summary(lm(Y ~ G))
  
  ## Naive and oracle estimates.
  naive.est <- summary(lm(X[R == 1] ~ G[R == 1]))
  oracle.est <- summary(lm(X ~ G))
  
  ## Inverse probability weighting.
  ipw.est1 <- ipw.linear(X = G, Y = X, R = R)
  ipw.est2 <- ipw.linear(X = G, Y = X, R = R, Z = Z)
  ipw.est3 <- ipw.linear(X = G, Y = X, R = R, Z = Z, drop.x = TRUE)
  ipw.est4 <- ipw.linear(X = G, Y = X, R = R, Z = Y)
  ipw.est5 <- ipw.linear(X = G, Y = X, R = R, Z = cbind(Y, Z))
  
  ## Heckman's method.
  Xobs <- X;  Xobs[R == 0] <- NA
  heck1 <- summary(selection(R ~ Z, Xobs ~ G, method = "2step"))
  heck2 <- summary(selection(R ~ Z, Xobs ~ G, method = "ml"))
  heck3 <- summary(selection(R ~ G + Z, Xobs ~ G, method = "2step"))
  heck4 <- summary(selection(R ~ G + Z, Xobs ~ G, method = "ml"))
  heck5 <- summary(selection(R ~ G + Y + Z, Xobs ~ G, method = "2step"))
  heck6 <- summary(selection(R ~ G + Y + Z, Xobs ~ G, method = "ml"))
  
  ## Partial likelihood MLE.
  partial.opt1 <- optim(par = rep(0, 3), partial.lik1, method = "BFGS", control = list(fnscale = -1), X = G, R = R, Z = Z)
  partial.opt2 <- optim(par = rep(0, 5), partial.lik2, method = "BFGS", control = list(fnscale = -1), X = G, R = R, Z = Z, Y = X, alpha.hat = partial.opt1$par, hessian = TRUE)
  partial.sd <- sqrt(diag(solve(- partial.opt2$hessian))[1:2])
  
  ## Full likelihood MLE.
  full.opt <- optim(par = rep(0, 8), full.lik, method = "BFGS", control = list(fnscale = -1), X = G, R = R, Z = Z, Y = X, hessian = TRUE)
  ...
  ## Save results.
  if (I %% 100 == 0) save(beta.x, gamma.x, beta.y, gamma.y, alpha.r, beta.r, gamma.r, delta.r, n, gy, oracle, naive,
                          partial, full, diagnostics, ipw1, ipw2, ipw3, ipw4, ipw5, 
                          heckman1, heckman2, heckman3, heckman4, heckman5, heckman6,
