...
for (I in 1:iter) {
  ...
  Y1 <- beta.y * X1 + gamma.y * U1 + rnorm(n1, 0, 1)   ## Used only for diagnostics.
  ...
  R2.probs <- expit(alpha.r + beta.r * X2 + gamma.r * Z2 + delta.r * Y2)   ## Same coefficients here although they could be different.
  ...
  if (I %% 100 == 0) save(beta.x, gamma.x, beta.y, gamma.y, alpha.r, beta.r, gamma.r, delta.r, n1, n2, diagnostics, 
                          ...
                          svyipw1.x, svyipw2.x, svyipw3.x, svyipw1.y, svyipw2.y, svyipw3.y, file = filename)
