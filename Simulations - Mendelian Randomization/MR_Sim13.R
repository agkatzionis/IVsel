...
filename <- "MR13_results.RData"

## Set up the data generation.
seed <- 67211383
iter <- 1000
n <- 10000
K <- 10   ## 10 SNPs seems reasonable for 10000 individuals.
n.boot <- 100   ## Bootstrap repetitions for 2SLS.

## Set values for the main simulation parameters.
gamma.x <- 1
beta.y <- 0.2   ## Causal effect of interest - tuned for power comparison.
gamma.y <- 1
beta.r <- 0   ## No X-R effect.
gamma.r <- 0.5   ## McFadden's R2 = 0.02.
delta.r <- 1   ## Semi-arbitrary (McFadden's R2 = 0.20-0.25).
alpha.r <- - (beta.r + beta.y * delta.r) / 2 * dnorm(3) / (1 - pnorm(3))   ## 50% of participants are selected.
