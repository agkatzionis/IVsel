...
filename <- paste("MrSimR1_5_XY_", range, "_results.RData", sep = "")

## Set up the values of the causal effect parameter.
n.values <- c(1e3, 2e3, 5e3, 2e4, 5e4, 1e5)   ## 1e4 is the default, so don't repeat.

## Set up the data generation.
seed <- 3551634
iter <- 1000   ## Use only 1000 iterations to save time - will be slow with large n.
n <- n.values[range]
