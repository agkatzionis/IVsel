...
beta.x <- sqrt(2/19)   ## Just use the same values as in Simulation 1.
gamma.x <- 1
beta.y <- 0.2
gamma.y <- 1
alpha.r <- 0
...
for (I in 1:iter) {
  ...
  Z <- rnorm(n, 0, 1)
  G <- rnorm(n, 0, 1)
  X <- beta.x * G + beta.x2 * G^2 + gamma.x * U + rnorm(n, 0, 1)
  ...
  ## Save results.
  if (I %% 100 == 0) save(beta.x, beta.x2, gamma.x, beta.y, gamma.y, alpha.r, beta.r, gamma.r, delta.r, n, gx, 
