...
##########   MR SIMULATION R1-1-XY   ##########

## Apply the IVsel methods in Mendelian randomization
## with a varying proportion of missing individuals.
...
filename <- paste("MrSimR1_1_XY_", range, "_results.RData", sep = "")

## Set up the intercept values to determine missingness rates.
alpha.r.values <- c(-2.91, -1.89, -0.86, 0.86, 1.89, 2.91)
## Tuned empirically to give the desired rates.
...
beta.y <- 0.2
gamma.y <- 1
alpha.r <- alpha.r.values[range]   ## The parameter to be varied.
beta.r <- 0.5
gamma.r <- 0.5
delta.r <- 0.5
